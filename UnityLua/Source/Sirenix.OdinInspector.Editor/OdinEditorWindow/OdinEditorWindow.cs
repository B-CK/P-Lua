...
namespace Sirenix.OdinInspector.Editor
{
    ...
    public class OdinEditorWindow : EditorWindow, ISerializationCallbackReceiver
    {
        ...
        public static OdinEditorWindow InspectObjectInDropDown(object obj, Rect btnRect, Vector2 windowSize)
        {
            ...
            if (windowSize.y == 0)
            {
                var tmpWindowSize = windowSize;
                window.messureArea = true;
                tmpWindowSize.y = 1;
                //windowSize.y = 400;
                // Render first couple of frames outside of the screen while we calculate the height.
                window.ShowAsDropDown(btnRect, tmpWindowSize);
                //var p = window.position;
                //window.position = new Rect(p.x, p.y, windowSize.x, 40);
                ...
                window.OnBeginGUI += () =>
                {
                    if (!isResized)
                    {
                        if (Event.current.type == EventType.Repaint)
                        {
                            ...
                            // It's really important that it actually repaints..
                        ...
                        {
                            window.messureArea = false;
                            window.ShowAsDropDown(btnRect, new Vector2(windowSize.x, Mathf.Min(400, (int)window.contentRect.height)));
                            isResized = true;
                        }
                    }
                };
        ...
        {
            bool endMessureArea = false;
            if (this.messureArea)
            {
                GUILayout.BeginArea(new Rect(0, 0, this.position.width, 1000));
                endMessureArea = true;
            }
