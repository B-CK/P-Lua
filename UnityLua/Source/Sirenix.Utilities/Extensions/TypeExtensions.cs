...
namespace Sirenix.Utilities
{
    ...
    public static class TypeExtensions
    {
        ...
        private static Func<Type, string> GetNiceNameProp
        {
            get
            {
                return getNiceName ?? (getNiceName = new Func<Type, string>(type =>
                {
                    if (type.IsArray)
                    {
        ...
        /// </summary>
        /// <remarks>
        /// <para>Note that in the special case of the type <see cref="UnityEngine.Quaternion"/>, a special equality comparer is returned that checks Quaternion's "q1 == q2", and then also checks whether all the Quaternion components are equal.</para>
        ...
        public static bool ImplementsOpenGenericClass(this Type candidateType, Type openGenericType)
        {
            ...
            if (openGenericType.IsGenericTypeDefinition == false || openGenericType.IsClass == false)
            {
        ...
        public static bool AreGenericConstraintsSatisfiedBy(this Type genericTypeDefinition, params Type[] parameters)
        {
            ...
            for (int i = 0; i < definitions.Length; i++)
            {
                ...
                if (!definition.TypeFulfillsGenericParameter(parameter, resolvedMap))
                {
                    return false;
                }
            }
        ...
        private static bool TypeFulfillsGenericParameter(this Type genericParameterDefinition, Type parameterType, Dictionary<Type, Type> resolvedMap, HashSet<Type> processedParams = null)
        {
            ...
            for (int i = 0; i < constraints.Length; i++)
            {
                ...
                {
                    if (!constraint.TypeFulfillsGenericParameter(parameterType, resolvedMap, processedParams))
                    {
                        return false;
                    }
                }
                else if (constraint.IsClass || constraint.IsInterface || constraint.IsValueType)
                {
                    if (constraint.IsGenericType)
                    {
                        ...
                        for (int j = 0; j < constraintParams.Length; j++)
                        {
                            ...
                            {
                                if (!processedParams.Contains(c) && !TypeFulfillsGenericParameter(c, p, resolvedMap, processedParams))
                                {
                                    return false;
                                }
                            }
